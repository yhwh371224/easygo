...
def render_email_template(template_name, context, request=None):
    # 이미 전체 경로이면 그대로
    if template_name.startswith("basecamp/emails/html_email/"):
        full_path = template_name
    else:
        full_path = f"{EMAIL_TEMPLATE_BASE}{template_name}"

    return render_to_string(full_path, context, request=request)
