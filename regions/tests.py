...
# ---------------------------------------------------------------------------
# View: region_booking (GET)
# ---------------------------------------------------------------------------

class RegionBookingViewTests(TestCase):

    def setUp(self):
        self.client = Client()
        self.region = make_region(slug='bktest', name='Booking Test')
        RegionSuburb.objects.create(
            region=self.region, name='Suburb A', slug='suburb-a', price=60, zone='Z1'
        )

    def test_get_booking_page_returns_200(self):
        url = reverse('regions:booking', kwargs={'region_slug': 'bktest'})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'regions/booking/booking.html')

    def test_suburbs_in_context(self):
        url = reverse('regions:booking', kwargs={'region_slug': 'bktest'})
        response = self.client.get(url)
        self.assertEqual(len(response.context['suburbs']), 1)

    @patch('regions.views.booking.send_telegram_notification', new_callable=AsyncMock)
    @patch('blog.bird_proxy.create_bird_mapping', return_value=True)
    @patch('blog.bird_proxy.close_bird_mapping', return_value=True)
    def test_post_creates_booking(self, mock_close, mock_create, mock_telegram):
        from blog.models import Post as Booking
        url = reverse('regions:booking_detail', kwargs={'region_slug': 'bktest'})
        data = {
            'name': 'John',
            'contact': '0412345678',
    ...
    @patch('blog.bird_proxy.create_bird_mapping', return_value=True)
    @patch('blog.bird_proxy.close_bird_mapping', return_value=True)
    def test_post_with_past_date_returns_error(self, mock_close, mock_create, mock_telegram):
        url = reverse('regions:booking_detail', kwargs={'region_slug': 'bktest'})
        data = {
            'name': 'Jane',
            'contact': '0412345678',
            ...
            'return_pickup_date': '',
            'pickup_time': '10:00',
            'direction': 'Drop off to Domestic Terminal',
            'suburb': 'suburb-a',
            'no_of_passenger': '1',
            'cf-turnstile-response': 'XXXX',
        }
