...
크론 — 가장 이른 도착이 아침 5시라 발송창은 04시부터다. 발송 시각은 "창이 열린 뒤
첫 실행"이라 30분 간격이면 도착 60~30분 전에 나간다.

    */30 4-22 * * * .../python manage.py arrival_reminder >> logs/arrival_reminder.log 2>&1

끝 시각(22시)이 시작 시각보다 중요하다. 창이 마지막 실행 뒤에 열리면 다음 실행 때는
pickup_date 가 어제라 대상에서 빠져 영영 발송되지 않는다(22:30 실행 = 23:30 도착까지
커버). 반대로 창이 첫 실행보다 이른 건은 04:00 실행이 늦게라도 보내고 지연 알림을 띄운다.
