...
def notify_user(email):
    ...
    if user and user.return_flight_number:
        p = Post(
            name=user.name, 
            contact=user.contact, 
            email=user.email, 
            flight_date=user.return_flight_date,
            flight_number=user.return_flight_number, 
            flight_time=user.return_flight_time, 
        ...
        html_content = render_to_string("basecamp/html_email-confirmation-return.html",
                                        {'name': user.name, 'contact': user.contact, 'email': user.email,
                                         'flight_date': user.return_flight_date, 'flight_number': user.return_flight_number,
                                         'flight_time': user.return_flight_time, 'pickup_time': user.pickup_time,
        ...
        email = EmailMultiAlternatives(
            ...
            text_content,
            '',
            [user.email, 'info@easygoshuttle.com.au']
        )
